 ...
 * <code>MapKeyJoinColumn</code> annotation is specified, a single join column is assumed and the default
 * values apply.
 * <p>
 * <pre>
 *
...
public @interface MapKeyJoinColumn {
   ...
   * upon the context.
   * <ul>
   * <li>If the join is for a map key for an element collection, the foreign key column is in the collection
   * table for the map value.
   * <li>If the join is for a map key for a ManyToMany entity relationship or for a OneToMany entity
   * relationship using a join table, the foreign key column is in a join table.
   * <li>If the join is for a OneToMany entity relationship using a foreign key mapping strategy, the
   * foreign key column for the map key is in the table of the entity that is the value of the map.
   * </ul>
   * <p>
   * Default (only applies if a single join column is used.) The concatenation of the following: the name of
   * the referencing relationship property or field of the referencing entity or embeddable class; "_";
   * "KEY".
   *
   * @return The name
   */
  ...
  /**
   * (Optional) The name of the column referenced by this foreign key column. The referenced column is in
  ...
  /**
   * (Optional) The SQL fragment that is used when generating the DDL for the column. Defaults to SQL
   * generated by the provider for the column.
   *
   ...
   * Default:
   * <ul>
   * <li>If the map is for an element collection, the name of the collection table for the map value.
   * <li>If the map is for a OneToMany or ManyToMany entity relationship using a join table, the name of the
   * join table for the map.
   * <li>If the map is for a OneToMany entity relationship using a foreign key mapping strategy, the name of
