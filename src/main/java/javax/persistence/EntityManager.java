 ...
 * <p>
 * An <code>EntityManager</code> instance is associated with a persistence context. A persistence context is a
 ...
 * <code>EntityManager</code> API is used to create and remove persistent entity instances, to find entities
 * by their primary key, and to query over entities.
 * <p>
 * The set of entities that can be managed by a given <code>EntityManager</code> instance is defined by a
...
public interface EntityManager {
   ...
   *                                      <code>EntityExistsException</code> or another <code>PersistenceException</code> may be thrown at flush
   *                                      or commit time.)
   ...
   *                                  argument is is not a valid type for that entitys primary key or is null
   */
   ...
   * obtaining the database lock. If these checks fail, the <code>OptimisticLockException</code> will be
   * thrown.
   * <p>
   ...
   * database, the <code>EntityNotFoundException</code> is thrown when the instance state is first accessed.
   * (The persistence provider runtime is permitted to throw the <code>EntityNotFoundException</code> when
   * <code>getReference</code> is called.) The application should not expect that the instance state will be
   ...
   * these checks fail, the <code>OptimisticLockException</code> will be thrown.
   * <p>
   * If the lock mode type is pessimistic and the entity instance is found but cannot be locked:
   * <ul>
   * <li>the <code>PessimisticLockException</code> will be thrown if the database locking failure causes
   ...
   *                                      <code>EntityManager</code> instance with <code>PersistenceContextType.TRANSACTION</code> or with a lock
   *                                      mode other than <code>NONE</code>
   * @throws EntityNotFoundException      if the entity no longer exists in the database
   * @throws PessimisticLockException     if pessimistic locking fails and the transaction is rolled back
   * @throws LockTimeoutException         if pessimistic locking fails and only the statement is rolled back
   * @throws PersistenceException         if an unsupported lock call is made
  ...
  /**
   * Create an instance of <code>TypedQuery</code> for executing a Java Persistence query language
  ...
  /**
   * Create an instance of <code>Query</code> for executing a named query (in the Java Persistence query
  ...
  /**
   * Create an instance of <code>TypedQuery</code> for executing a Java Persistence query language named
   * query. The select list of the query must contain only a single item, which must be assignable to the
   * type specified by the <code>resultClass</code> argument.
   *
   * @param name        the name of a query defined in metadata
   * @param resultClass the type of the query result
   * @param <T>         Type
  ...
  /**
   * Create an instance of <code>Query</code> for executing a native SQL statement, e.g., for update or
   * delete.
   *
   ...
   *            <code>EntityManager</code> implementation class or an interface that it implements.
   * @param <T> Type
  ...
  /**
   * Return the underlying provider object for the <code>EntityManager</code>, if available. The result of
   * this method is implementation specific. The <code>unwrap</code> method is to be preferred for new
   * applications.
   *
   ...
   * the <code>EntityManager</code> instance and any <code>Query</code> and <code>TypedQuery</code> objects
   * obtained from it will throw the <code>IllegalStateException</code> except for
   * <code>getProperties</code>, <code>getTransaction</code>, and <code>isOpen</code> (which will return
  ...
  /**
   * Return the resource-level <code>EntityTransaction</code> object. The <code>EntityTransaction</code>
  ...
  /**
   * Return an instance of <code>Metamodel</code> interface for access to the metamodel of the persistence
   * unit.
   *
