...
public interface EntityManagerFactory {
  ...
  /**
   * Create a new application-managed <code>EntityManager</code> with the specified Map of properties. This
   * method returns a new <code>EntityManager</code> instance each time it is invoked. The
   * <code>isOpen</code> method will return true on the returned instance.
   *
   ...
   * properties. This method returns a new EntityManager instance each time it is invoked. The isOpen method
  ...
  /**
   * Close the factory, releasing any resources that it holds. After a factory instance has been closed, all
   * methods invoked on it will throw the <code>IllegalStateException</code>, except for <code>isOpen</code>
   * , which will return false. Once an <code>EntityManagerFactory</code> has been closed, all its entity
  ...
  /**
   * Get the properties and associated values that are in effect for the entity manager factory. Changing
   ...
   * objects can be created from it using the createNamedQuery methods. Any configuration of the query
   * object (except for actual parameter binding) in effect when the named query is added is retained as
   * part of the named query definition. This includes configuration information such as max results, hints,
   * flush mode, lock mode, result set mapping information, and information about stored procedure
   * parameters. When the query is executed, information that can be set by means of the Query API can be
   * overridden. Information that is overridden does not affect the named query as registered with the
   * entity manager factory, and thus does not affect subsequent query objects created from it by means of
   * the createNamedQuery method. If a named query of the same name has been previously defined, either
   * statically via metadata or via this method, that query definition is replaced.
   *
   ...
   * EntityManagerFactory implementation does not support the specified class, the PersistenceException is
   * thrown.
   *
  ...
  /**
   * Add a named copy of the EntityGraph to the EntityManagerFactory. If an entity graph with the same name
